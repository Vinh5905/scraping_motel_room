...
def update_crawl_info(PATH):
    upload_file(os.getenv('FOLDER_BATDONGSAN_STORAGE_ID'), PATH['CRAWL'].name, file_path=PATH['CRAWL'], type='replace')
    print_banner_colored('Upload crawl info thành công', 'success')

def update_extract_info(PATH):
    upload_file(os.getenv('FOLDER_BATDONGSAN_STORAGE_ID'), PATH['EXTRACT'].name, file_path=PATH['EXTRACT'], type='replace')
    print_banner_colored('Upload extract info thành công', 'success')


def update_link_list(PATH):
    upload_file(os.getenv('FOLDER_BATDONGSAN_STORAGE_ID'), PATH['LINK_LIST'].name, file_path=PATH['LINK_LIST'], type='replace')
    print_banner_colored('Upload link list thành công', 'success')


def pull_from_onedrive(PATH):
    previous_crawl_info = search_file_id(PATH['CRAWL'].name, folder_id=os.getenv('FOLDER_BATDONGSAN_STORAGE_ID'))
    previous_extract_info = search_file_id(PATH['EXTRACT'].name, folder_id=os.getenv('FOLDER_BATDONGSAN_STORAGE_ID'))
    previous_link_list = search_file_id(PATH['LINK_LIST'].name, folder_id=os.getenv('FOLDER_BATDONGSAN_STORAGE_ID'))
